...
# 路径相关设置
parser.add_argument('--img_path', type=str,help='影像文件路径', default=r"F:\2021\3——算法研究\车辆检测\1—阿布扎比车辆\谷歌影像")
parser.add_argument('--geojson_path', type=str,help='样本geojson文件路径', default=r"F:\2021\3——算法研究\车辆检测\1—阿布扎比车辆\样本\merge.json")

parser.add_argument('--class_field', type=str,help='样本geojson文件中类别字段名', default=r"category")

parser.add_argument('--output_dir', type=str,help='文件输出路径', default=r"F:\2021\3——算法研究\车辆检测\1—阿布扎比车辆\labelme")
parser.add_argument('--suffix', type=str,help='输出图片后缀', default=r".png")
# parser.add_argument('--binary', type=bool,help='是否在json里面写入影像字节', default=False)
parser.add_argument('--bands', type=int,help='输出图片的波段数选择，如果影像有这么多波段的话', default=3)
# parser.add_argument('--bands_order', type=int,help='输出图片的通道顺序，0代表rgb，1代表bgr(nir)', default=0)

# 裁剪相关参数设置
parser.add_argument('--overlap_ratio', type=float,help='裁剪重叠率',default=0.2)
...
def is_out_of_image(polygon, width, height):
    """
    判断一个imgaug格式的polygon是否在图像上,只有有一部分不在图像上就返回True
    参数：
    polygon————待判断的polygon，其坐标已经shift到当前图像xy
    width————图像宽
    height————图像高
    """
...
def IMG_CLIP(image_path, geojson_path):
    ...
    with open(geojson_path, 'r',encoding='utf-8') as f:
        ...
        for feature in instance['features']:
            if feature["geometry"]["type"] != "Polygon" and feature["geometry"]["type"] != "MultiPolygon":
                ...
                features_points_list.append(feature["geometry"]['coordinates'][0])
    ...
    for clip_box in tqdm(clip_box_list):
        ...
        # 存储图片
        out_img_name = osp.basename(image_path).split(".")[0] + "_clip_" + str(block_size) + '_' + str(clip_box_index) + config.suffix
        output_dir = osp.join(config.output_dir, "img")
        if not os.path.exists(output_dir):
            os.mkdir(output_dir)
        out_img_path = osp.join(output_dir, out_img_name)
        # 存储json
        out_json_name = osp.basename(image_path).split(".")[0] + "_clip_" + str(block_size) + '_' + str(clip_box_index) + ".json"
        output_dir = osp.join(config.output_dir, "img")
        if not os.path.exists(output_dir):
            os.mkdir(output_dir)
        ...
        for polygon in polygon_list_shift:
            if not is_out_of_image(polygon, width, height):
...
def IMG_CLIP_E(image_path, geojson_path):
    '''
    IMG_CLIP的加强：
    （x）选择性将图像字节写入json
    （o）同时支持3波段和4波段
    （o）输出影像颜色深度选择（8bit 12bit 16bit）
    ...
    with open(geojson_path, 'r', encoding='utf-8') as f:
        ...
        for feature in instance['features']:
            if feature["geometry"]["type"] != "Polygon" and feature["geometry"]["type"] != "MultiPolygon":
                ...
                features_points_list.append(feature["geometry"]['coordinates'][0])
                features_class_list.append(feature['properties'][config.class_field])
    # features_xy_points_list = []
    polygon_list = []
    for i, feature_points in enumerate(features_points_list):
        xy_points_list = []
        for lonlat in feature_points:
            xy = lonlat2imagexy(dataset, float(lonlat[0]), float(lonlat[1]))
            xy_points_list.append(xy)
        # features_xy_points_list.append(xy_points_list)
        # 构造带label的Polygon对象，添加到列表中
        polygon_list.append(Polygon(xy_points_list, label=features_class_list[i]))
    ...
    for clip_box in tqdm(clip_box_list):
        ...
        height = block_size
        # 存储图片
        out_img_name = osp.basename(image_path).split(".")[0] + "_clip_" + str(block_size) + '_' + str(
            clip_box_index) + config.suffix
        output_dir = osp.join(config.output_dir, "img")
        if not os.path.exists(output_dir):
            os.mkdir(output_dir)
        out_img_path = osp.join(output_dir, out_img_name)
        # 存储json
        out_json_name = osp.basename(image_path).split(".")[0] + "_clip_" + str(block_size) + '_' + str(
            clip_box_index) + ".json"
        output_dir = osp.join(config.output_dir, "img")
        if not os.path.exists(output_dir):
            os.mkdir(output_dir)
        ...
        # 多边形的坐标平移到当前分块patch,移除了不在当前分块的多边形
        polygon_list_shift = list(map(lambda x: x.shift(top=-ymin, left=-xmin), polygon_list))
        ...
        polygon_list_shift = polygon_list_shift_
        aug_polygon_list = polygon_list_shift
        # 如果dataset类型是Byte且波段数是3
        if img_data_type == 'Byte' and img_bands == 3:
            polygons_num = len(aug_polygon_list)
            # print(polygons_num)
            if polygons_num == 0:
                # 裁剪后没有多边形 直接跳出 不做文件存储
                ...
                print(f"当前分块有{polygons_num}个多边形")
                if not os.path.exists(out_img_path):
                    img_data_int8 = dataset.ReadAsArray(xmin, ymin, width, height) # 获取分块数据
                    img_data = np.transpose(img_data_int8, (1, 2, 0))#[:, :, [2, 1, 0]]
                    skio.imsave(out_img_path, img_data)
                else:
                    print(f"文件{out_img_path}已存在，跳过存储")
                ...
                if not os.path.exists(out_json_path):
                    ...
                    for aug_polygon in aug_polygon_list:
                        ...
                        json_shape["bbox"] = [x_min, y_min, width, height]
                        # 类别字段赋值
                        json_shape["label"] = CATEGORIES[aug_polygon.label]
                ...
                clip_box_index += 1
        # dataset是Byte且波段数是4
        elif img_data_type == 'Byte' and img_bands == 4:
            # 只输出三波段
            if config.bands == 3:
                # clip_box_index = 0
                # for clip_box in tqdm(clip_box_list):
            ...
            # 输出所有波段
                ...
                #     aug_polygon_list = polygon_list_shift
                    ...
                    if polygons_num == 0:
                        ...
                        print(f"当前分块有{polygons_num}个多边形")
                        if not os.path.exists(out_img_path):
                            img_data = dataset.ReadAsArray(xmin, ymin, width, height) # 获取分块数据
            ...
            print(f'Not spported data type{img_data_type} or band number {img_bands}')
...
if __name__ == '__main__':
    ...
    if not osp.exists(geojson_path):
        ...
        exit()
    # 生成tif-json tuple对的list，作为starmap函数的迭代变量，an iterable of [(1,2), (3, 4)] results in [func(1,2), func(3,4)].
    ...
    with tqdm(total=len(tif_file_list_with_json)) as t:
        for _ in pool.starmap(IMG_CLIP_E, tif_file_list_with_json):
            t.update(1)
    # IMG_CLIP_E(tif_file_list[0], geojson_path)
    print("处理完毕~ ~ ~")
    end_time = time.time()
